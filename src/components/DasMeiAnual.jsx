...
export default function DasMeiAnual({ cliente }) {
  ...
  async function enviarAgora(guia) {
    const mensagem = `Olá, ${cliente.nome}. Seu DAS-MEI de ${nomeCompetencia(guia.competencia)} está disponível. Valor: ${moeda(guia.valor)}. Vencimento: ${dataBR(guia.vencimento)}.`
    ...
    <section id="central-das-mei" style={box}>
      <div style={topo}>
        <div>
          ...
          <p style={descricao}>Importe as guias mensais do PGMEI, organize por competência e programe o envio ao cliente.</p>
        </div>
        ...
        <table style={tabela}>
          ...
          <tbody>
            {guias.map((guia) => <tr key={guia.id}>
              <td>{nomeCompetencia(guia.competencia)}</td>
              <td>{dataBR(guia.vencimento)}</td>
              <td>{moeda(guia.valor)}</td>
              <td><input style={dataInput} type="date" value={guia.dataProgramadaEnvio || ""} onChange={(e) => atualizar(guia, { dataProgramadaEnvio: e.target.value })} /></td>
              <td><span style={status}>{guia.statusCalculado || guia.status}</span></td>
      ...
      <p style={nota}>O agendamento coloca a guia na fila “Pronta para envio”. O envio pelo WhatsApp é confirmado ao abrir a conversa.</p>
    </section>
  )
}
...
const dataInput = { border: "1px solid #cbd5e1", borderRadius: 7, padding: 7 }
