...
const destinatarioVazio = { nome: "", cpfCnpj: "", inscricaoEstadual: "", email: "", cep: "", endereco: "", numero: "", bairro: "", cidade: "", estado: "" }
...
export default function NFe() {
  ...
  const [config, setConfig] = useState({ serie: 1, proximoNumero: 1, crt: "", naturezaOperacao: "Venda de mercadoria", certificadoDigitalId: "" })
  ...
  const certificadosEmitente = certificados.filter((item) => String(item.clienteId) === String(clienteId) && item.ativo !== false)
  ...
  return <section className="nfe-page">
    ...
    {aba === "config" && <div className="nfe-grid">
      <div className="nfe-card"><h3>Emissão</h3><label>Ambiente<input value="Homologação" disabled /></label><label>Série<input type="number" value={config.serie || 1} onChange={(e) => setConfig({ ...config, serie: e.target.value })} /></label><label>Próximo número<input type="number" value={config.proximoNumero || 1} onChange={(e) => setConfig({ ...config, proximoNumero: e.target.value })} /></label><label>CRT<select value={config.crt || ""} onChange={(e) => setConfig({ ...config, crt: e.target.value })}><option value="">Selecione</option><option value="1">1 — Simples Nacional</option><option value="2">2 — Simples, excesso sublimite</option><option value="3">3 — Regime Normal</option><option value="4">4 — MEI</option></select></label><label>Natureza da operação<input value={config.naturezaOperacao || ""} onChange={(e) => setConfig({ ...config, naturezaOperacao: e.target.value })} /></label><label>Certificado A1<select value={config.certificadoDigitalId || ""} onChange={(e) => setConfig({ ...config, certificadoDigitalId: e.target.value })}><option value="">Selecione</option>{certificadosEmitente.map((c) => <option value={c.id} key={c.id}>{c.nomeArquivo || c.numeroSerie || `Certificado #${c.id}`} · vence {c.dataValidade}</option>)}</select></label><button className="primary" onClick={salvarConfiguracao}>Salvar configuração</button></div>
      <div className="nfe-card"><h3>Diagnóstico</h3>{diagnostico?.pendencias?.map((p) => <div className="nfe-pendency" key={p}>⚠ {p}</div>)}{diagnostico?.prontoParaRascunho && <div className="nfe-ok">Cadastro pronto para rascunhos.</div>}</div>
    </div>}
