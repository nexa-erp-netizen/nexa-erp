...
export default function FiscalCliente() {
  ...
  async function carregarFiscal() {
    ...
    try {
      ...
      console.error("ERRO FISCAL CLIENTE:", error)
      setFiscal([])
    } finally {
      setCarregando(false)
    }
  ...
  function obterTitulo(item) {
    return item.obrigacao || item.titulo || item.descricao || "Guia fiscal"
  }
  ...
  function estaVencido(item) {
    if (!item.vencimento || estaPago(item)) return false
  ...
  async function baixarGuia(item) {
    try {
      ...
      if (!url) {
        alert("Nenhuma guia disponível para baixar.")
        return
      }
      ...
      console.error(error)
      alert("Erro ao baixar guia.")
    }
  }

  async function marcarPago(item) {
    const confirmar = window.confirm(
      "Confirmar que você já pagou esta guia?"
    )
  ...
  return (
    <div className="fc-page">
      ...
      <div className="fc-list">
        {carregando ? (
          <div className="fc-empty">Carregando guias fiscais...</div>
        ) : guias.length === 0 ? (
          <div className="fc-empty">
            Nenhuma guia fiscal disponível no momento.
          </div>
        ) : (
