...
export default function PendenciasClientes() {
  ...
  const guiasVencidas = guiasFiscais.filter(guiaEstaVencida)
  const guiasEmAndamento = guiasFiscais.filter((item) => !guiaEstaVencida(item))
  ...
  return (
    <div className="pd-page">
      <style>{`
        ...
        .pd-guide-progress { background: rgba(60,188,255,.14); color: #72d2ff; }
      ...
      <div className="pd-card">
        <div className="pd-card-title">Acompanhamento de guias enviadas ({guiasEmAndamento.length})</div>
        <div className="pd-table-wrapper">
          <table className="pd-table">
            <thead><tr><th>Cliente</th><th>Obrigação</th><th>Competência</th><th>Vencimento</th><th>Valor</th><th>Andamento</th></tr></thead>
            <tbody>
              {guiasEmAndamento.map((item) => {
                const situacao = situacaoGuia(item)
                return <tr key={`andamento-${item.id}`}><td>{item.cliente}</td><td>{item.obrigacao || item.descricao || "Guia fiscal"}</td><td>{item.competencia || "-"}</td><td>{formatarData(String(item.vencimento || "").slice(0, 10))}</td><td>{valorGuia(item.valor)}</td><td><span className="pd-guide-status pd-guide-progress">Enviada ao cliente • aguardando pagamento • {situacao}</span></td></tr>
              })}
              {!guiasEmAndamento.length && <tr><td colSpan="6" className="empty">Nenhuma guia aguardando pagamento.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="pd-table-wrapper">
          <table className="pd-table">
            ...
            <tbody>
              ...
              {!guiasVencidas.length && <tr><td colSpan="6" className="empty">Nenhuma guia ou obrigação vencida.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
